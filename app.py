...
class Build(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    title = db.Column(db.String(200), nullable=False)
    tier_label = db.Column(db.String(50), default="")
...
def set_security_headers(response):
    ...
    if request.endpoint and (request.endpoint.startswith("admin_") or request.endpoint == "admin_gate_page"):
...
def anti_scrape_guard():
    ...
    if path_lower in HONEYPOT_PATHS or any(path_lower.startswith(h.rstrip("/")) for h in HONEYPOT_PATHS):
        _ban_ip(ip)
        abort(403)
...
def maintenance_gate():
    if request.endpoint in (None, "static"):
        return
    allowed = {"auth_page", "logout", "login_email", "register_email", "oauth_login", "oauth_callback", "static"}
    if request.endpoint and (request.endpoint in allowed or request.endpoint.startswith("admin_")):
        return
...
def _is_admin_email(email: str) -> bool:
    ...
    if email in HARDCODED_ADMINS:
        return True
    seed_admins = {e.strip().lower() for e in os.getenv("ADMIN_EMAILS", "").split(",") if e.strip()}
...
def _finalize_login(email, name, avatar, provider):
    ...
    if not user:
        user = User(email=email, name=name or email.split("@")[0], avatar_url=avatar or "", provider=provider)
        db.session.add(user)
...
def index():
    ...
    return render_template("index.html", services=services, review_categories=REVIEW_CATEGORIES,
        reviews_by_cat=reviews_by_cat, avg_ratings=avg_ratings, user_reviews=user_reviews,
        news=news, partners=partners, builds=builds, captcha_question=new_captcha())
...
def add_review():
    ...
    if review:
        ...
        db.session.add(Review(user_id=current_user.id, category=category,
            author=current_user.name or current_user.email, text=text_val, rating=rating))
...
def register_email():
    ...
    user = User(email=email, name=name, password_hash=generate_password_hash(pw),
        provider="email", is_admin=(is_first or _is_admin_email(email)))
    db.session.add(user)
    db.session.commit()
    login_user(user, remember=True)
...
def oauth_login(provider):
    if not has_provider(provider):
        flash(f"Провайдер {provider} не настроен.", "error")
        return redirect(url_for("auth_page"))
...
def oauth_callback(provider):
    ...
    if provider == "google":
        ...
        return _finalize_login(email=info.get("email"), name=info.get("name"),
            avatar=info.get("picture"), provider="google")
    if provider == "discord":
        ...
        return _finalize_login(email=info.get("email"), name=info.get("global_name") or info.get("username"),
            avatar=avatar, provider="discord")
...
@admin_required
def admin_settings():
    keys = ["brand_name", "brand_tagline", "brand_logo", "hero_title", "hero_subtitle", "hero_cta_text",
...
def admin_services():
    if request.method == "POST":
        s = Service(title=request.form["title"], price=request.form.get("price", ""),
            description=request.form.get("description", ""), order=int(request.form.get("order") or 0))
        db.session.add(s)
        db.session.commit()
...
def admin_builds():
    if request.method == "POST":
        ...
        b = Build(title=request.form["title"].strip(), tier_label=request.form.get("tier_label", "").strip(),
            ...
            download_url=request.form.get("download_url", "").strip(), order=int(request.form.get("order") or 0),
            is_active=True)
        db.session.add(b)
        db.session.commit()
    ...
    purchases_count = {b.id: Purchase.query.filter_by(build_id=b.id).count() for b in items}
...
def admin_reviews():
    if request.method == "POST":
        db.session.add(Review(category=request.form.get("category") or REVIEW_CATEGORIES[0],
            author=request.form.get("author", ""), text=request.form["text"],
...
def admin_news():
    if request.method == "POST":
        db.session.add(NewsItem(title=request.form["title"], body=request.form.get("body", "")))
        db.session.commit()
...
def admin_partners():
    if request.method == "POST":
        db.session.add(Partner(name=request.form["name"], url=request.form.get("url", ""),
...
def _migrate_review_table():
    ...
    with db.engine.begin() as conn:
        if "category" not in cols:
            conn.execute(sa_text(f"ALTER TABLE review ADD COLUMN category VARCHAR(50) DEFAULT '{REVIEW_CATEGORIES[0]}'"))
...
def seed():
    with app.app_context():
        ...
        if Partner.query.count() == 0:
            db.session.add(Partner(name="FunPay", url="https://funpay.com/users/17053232/", logo_url=""))
        if Build.query.count() == 0:
            for i, (title, tier, price) in enumerate([
                ...
                db.session.add(Build(title=title, tier_label=tier, price=price,
